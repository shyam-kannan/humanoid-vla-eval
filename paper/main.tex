...
further as the task progresses through transport into retreat (Fig.~\ref{fig:phase}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_phase_breakdown.pdf}
\caption{Classification breakdown by sub-goal phase (Table~\ref{tab:byphase}). Failure
concentrates and grows through the task, from reach to transport to retreat.}
\label{fig:phase}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_match_distribution.pdf}
\caption{How many of the 5 scored dimensions land in the strictest match tier
simultaneously, per phase (Table~\ref{tab:matchcount}). The distribution centers at
1--2/5 rather than clustering near 4/5, evidence against a single miscalibrated
threshold being responsible for the 0\% strict-success rate.}
\label{fig:matchdist}
\end{figure}

% [TODO if time permits: example frame sequence for a representative
% intent-lost-in-handoff case with predicted-vs-actual overlay]
...
Table~\ref{tab:matchcount} and Fig.~\ref{fig:matchdist}'s distribution supports this
being a real measurement of partial trajectory fidelity rather than a rubric artifact --
most phases achieve 1--2 of 5 dimensions within the strictest tolerance, not zero, and the
